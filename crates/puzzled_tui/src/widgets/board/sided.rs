...
use crate::{
    CellRender, GridOptions, GridRenderState, LineRender, RenderSize, Viewport, align_area,
    align_vertically,
};
use derive_more::{Debug, Deref, DerefMut};
use puzzled_core::{Direction, Position, SidedGrid};
use ratatui::{
...
// pub struct SidedGridWidget<'a, T, U, S> {
//     pub grid: &'a SidedGrid<T, U>,
//     pub render_state: &'a SidedGridState,
//
//     _marker: PhantomData<S>,
// }
//
// impl<'a, T, U, S> RenderSize for SidedGridWidget<'a, T, U, S> {
...
//         let grid_state = GridRenderState::from(*self.render_state);
//         let grid = GridWidget::<'a, T, S>::new(&self.grid.grid, &grid_state);
//
...
//
//         // Vertical sides
//         if self.grid.top.is_some() {
//             size.height += opts.top_height;
//         }
//         if self.grid.bottom.is_some() {
//             size.height += opts.bottom_height;
//         }
//
//         // Horizontal sided
//         if self.grid.left.is_some() {
//             size.width += opts.left_width;
//         }
//         if self.grid.right.is_some() {
...
// }
//
// impl<'a, T, U, S> StatefulWidget for SidedGridWidget<'a, T, U, S>
// where
//     T: CellRender<S>,
...
//     type State = S;
//
//     fn render(self, parent: Rect, buf: &mut Buffer, cell_state: &mut Self::State) {
...
//
//         // Render the grid
//         let grid = GridWidget::new(&self.grid.grid, &self.render_state);
//
//         grid.render(grid_area, buf, cell_state);
//
//         // Render the sides
